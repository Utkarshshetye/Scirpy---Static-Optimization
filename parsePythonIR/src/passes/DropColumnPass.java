...
//            System.out.println("\t att live after: " + set);
            ...
            // Utkarsh: To store initial mapping, used in ReadOnlyAttributeAnalysis
//            InputFileDataTypeMapper inpFileData = getPdLists().get(0).getIfdtm();
//            MetaData data = inpFileData.getMd();
//            PythonScene.initialCols.addAll(data.getClmnList());
...
//        }
        ...
        for(String clmn:md.getClmnList()) {
            ...
            if (md.getIsCategory()[i] == true && md.getClmnTypeMap().get(clmn).equals("str")) {
                ...
                if (!isUsedInExpression(clmn)){
                    dict.getValue().add(new Str(0, 0, "category"));
                }else {
                    dict.getValue().add(new Str(0, 0, "str"));
                }
            }
...
//        }
    ...
    private boolean isUsedInExpression(String clmn){
        ...
        while (unitIterator.hasNext()) {
            ...
            if(unit instanceof AssignmentStmtSoot) {
                AssignmentStmtSoot assignmentStmtSoot = (AssignmentStmtSoot) unit;
                AssignStmt assignStmt = assignmentStmtSoot.getAssignStmt();
                IExpr assignRHS = assignStmt.getRHS();

                if(assignRHS instanceof BinOp) {
                    String leftCol = "", rightCol = "";
                    BinOp binOp = (BinOp) assignStmt.getRHS();

                    if (binOp.getLeft() instanceof Subscript) {
                        Subscript l1 = (Subscript) binOp.getLeft();
                        Slice leftSlice = l1.getSlice();

                        if (leftSlice.getIndex() instanceof Constant) {
                            Constant c = (Constant) leftSlice.getIndex();
                            leftCol = c.getValue();
                        }
                    }

                    if (binOp.getRight() instanceof Subscript) {
                        Subscript l2 = (Subscript) binOp.getRight();
                        Slice rightSlice = l2.getSlice();

                        if (rightSlice.getIndex() instanceof Constant) {
                            Constant c = (Constant) rightSlice.getIndex();
                            rightCol = c.getValue();
                        }
                    }

                    if (leftCol.equals(clmn) || rightCol.equals(clmn)) {
                        return true;
                    }
                } else if (assignRHS instanceof Call) {
                    Call call = (Call) assignRHS;
                    IExpr callAttr = call.getFunc();
                    if (callAttr instanceof Attribute) {
                        Attribute dfAttribute = (Attribute) callAttr;
                        IExpr definedOn = dfAttribute.getValue();

                        if (definedOn instanceof Subscript) {
                            // Operations like: df['name'].count() or df['name'].str.column,
                            // if name defined as a category type, gives an error
                            Subscript colDefineOn = (Subscript) definedOn;
                            Slice definedOnSlice = colDefineOn.getSlice();

                            if (definedOnSlice.getIndex() instanceof Constant) {
                                Constant colConst = (Constant) definedOnSlice.getIndex();
                                String colName = colConst.getValue();

                                if (colName.equals(clmn)) {
                                    return true;
                                }
                            }
                        }
                ...
                else if (assignRHS instanceof Attribute) {
                    Attribute assignAttr = (Attribute) assignRHS;
