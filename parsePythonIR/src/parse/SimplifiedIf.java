...
public class SimplifiedIf {
    ...
    public PatchingChain<Unit> getSimplifiedIf(IfStmtPy ifStmtPy, boolean whileBool, Chain locals){
        ...
        if(condition instanceof BoolOp){
            ...
            for(Object boolOp : ((BoolOp) condition).getValues()){
                IExpr op =  (IExpr) boolOp;
                addIfStmt(op, whileBool, ifStmtPy, extendedIfList, locals);
            }
        }else {
           addIfStmt(condition, whileBool, ifStmtPy, extendedIfList, locals);
       }
        return extendedIfList;

    }

    private void addIfStmt(IExpr condition, boolean whileBool, IfStmtPy ifStmtPy, PatchingChain<Unit> extendedIfList, Chain locals){
